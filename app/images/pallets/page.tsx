...
import { skus, users, userById, skuById, currentModelChain, palletThumb } from "@/mock/data";
import { PageHeader } from "@/components/ui/page-header";
import { StatusPill } from "@/components/ui/status-pill";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select } from "@/components/ui/select";
import { cn, formatAbsolute, formatPct } from "@/lib/utils";
import { Verdict } from "@/mock/types";
import Link from "next/link";
import { X, MessageSquareDiff } from "lucide-react";

const VERDICTS: Verdict[] = ["accept", "reject", "low_confidence", "unknownSku"];
...
export default function ImageRegisterPage() {
  ...
  const filters: ImageFilters = {
    ...
    requiredMitigation: sp.get("mitigation") === "1" || undefined,
  ...
  const setParam = (key: string, value: string | null | undefined) => {
    ...
    params.delete("page");
    router.replace(`/images/pallets?${params.toString()}`, { scroll: false });
  };
  ...
  const clearAll = () => router.replace("/images/pallets?range=today", { scroll: false });
  ...
  (filters.verdicts ?? []).forEach((v) => activeChips.push({ key: `verdict:${v}`, label: `Verdict: ${VERDICT_LABELS[v as Verdict] ?? v}`, onRemove: () => toggleInCsv("verdict", v) }));
  ...
  if (filters.requiredMitigation) activeChips.push({ key: "mitigation", label: "Required mitigation", onRemove: () => setParam("mitigation", null) });
    ...
    <div className="p-6 flex flex-col gap-4 max-w-[1600px]">
      ...
      <div className="card p-3 flex flex-col gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <FilterMultiSelect
            label="Verdict"
            options={VERDICTS.map((v) => ({ value: v, label: VERDICT_LABELS[v] }))}
          ...
          <ToggleChip active={!!filters.requiredMitigation} onClick={() => setParam("mitigation", filters.requiredMitigation ? null : "1")}>
            Required mitigation
          </ToggleChip>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border bg-subtle">
              {["", "Time", "SKU", "Count", "Verdict", "Average conf", "Required mitigation"].map((h) => (
          ...
          <tbody>
            {rows.map((p) => {
              ...
              return (
                <tr key={p.id} className="border-b border-border last:border-b-0 hover:bg-subtle transition cursor-pointer" onClick={() => router.push(`/images/pallets/${p.id}`)}>
                  <td className="px-3 py-2 w-14">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={palletThumb(p.skuId, p._imageSeed)} alt="" className="w-12 h-12 object-cover border border-border" style={{ borderRadius: 6 }} />
                  </td>
                  <td className="px-3 py-2 mono tabular text-sm text-muted-fg whitespace-nowrap">
                    {formatAbsolute(p.startedAt)}
                  </td>
                  <td className="px-3 py-2">
                    {sku ? (
                      <Link href={`/food_passport/${sku.id}`} className="group" onClick={(e) => e.stopPropagation()}>
                      ...
                      <span className="mono text-xs text-muted-fg uppercase">unknown</span>
                    )}
                  </td>
                  <td className="px-3 py-2 mono tabular text-sm">
                    {p.itemsDetected != null ? (
                      <span className={cn(p.countMismatch && "text-status-amber")}>
                        {p.itemsDetected}
                        {sku?.expectedCount != null && (
                          <span className="text-muted-fg"> / {sku.expectedCount}</span>
                        )}
                      </span>
                    ) : (
                  ...
                  <td className="px-3 py-2">
                    {p.requiredMitigation && (
                      <MessageSquareDiff className="w-3.5 h-3.5 text-status-amber" strokeWidth={1.5} />
                    )}
                  </td>
                </tr>
              );

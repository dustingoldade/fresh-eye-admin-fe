...
const GLOSSARY: GlossarySection[] = [
  ...
  {
    category: "Inspection units",
    blurb: "What gets inspected and what counts as good or bad.",
    entries: [
      {
        term: "Pallet",
        body:
  ...
  {
    category: "The model chain",
    blurb: "How the system sees a pallet.",
    entries: [
      {
        term: "Model Chain",
        body:
  ...
  {
    category: "The system's call",
    blurb: "What the model emits per pallet.",
    entries: [
      {
        term: "Verdict",
  ...
  },
  {
    ...
    blurb: "The voice flow that lets the operator overrule the model.",
    entries: [
      {
        term: "Wake Word",
        body:
  ...
  {
    category: "Headline metric",
    blurb: "The one number we report.",
    entries: [
      {
...
const TOTAL_TERMS = GLOSSARY.reduce((sum, s) => sum + s.entries.length, 0);
  ...
  return (
    <div className="p-6 flex flex-col gap-8 max-w-[1100px] mx-auto">
      ...
      <section className="card overflow-hidden">
        <SectionHeader title={`Terms · ${TOTAL_TERMS}`} />
        {GLOSSARY.map((section) => (
          <div key={section.category}>
            <div className="px-5 py-3 border-b border-border bg-subtle">
              <h2 className="text-sm font-semibold text-foreground">{section.category}</h2>
              <p className="text-xs text-muted-fg italic mt-0.5">{section.blurb}</p>
            </div>
